...
-- Export de données de la table sgtools.customerpacks : ~2 rows (environ)
...
-- Export de données de la table sgtools.material_liaison : ~134 rows (environ)
...
-- Export de données de la table sgtools.patient : ~8 rows (environ)
DELETE FROM `patient`;
...
INSERT INTO `patient` (`id`, `idCustomer`, `lastname`, `firstname`, `sex`, `age`, `height`, `weight`, `avatar`) VALUES
	...
	(3, 1, 'Barreau', 'Nathalie', 0, 26, 167, 72, 1),
...
-- Export de données de la table sgtools.patient_liaison : ~22 rows (environ)
...
-- Export de données de la table sgtools.questions_liaison : ~34 rows (environ)
...
-- Export de données de la table sgtools.surgery : ~8 rows (environ)
DELETE FROM `surgery`;
